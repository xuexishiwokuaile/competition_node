...
const apply = {
    ...
    findCaptainByTeam:
        "SELECT competition.name as competitionName, team.name as teamName, user.name as userName, user.id as userId, user.profile, position.positionName FROM apply, user, position, competition, team WHERE apply.teamId = ? AND apply.status = 2 AND apply.captain = user.id AND apply.teamId = team.id AND position.positionId = team.capPositionId AND apply.comId = competition.id ",
    findMemberByTeam:
    ...
    findReceive:
        "SELECT apply.status, team.name as teamName, competition.name as competitionName, user.name as userName, user.id as userId, user.profile FROM apply, team, user, competition WHERE apply.captain = ? AND apply.teamId = team.id AND team.comId = competition.id AND apply.member = user.id", // 收到的
};
